"""Tests for Story building, launch-asset synthesis, and post message assembly.
...
from media_engine.models import DevEvent, LaunchAsset
...
def _assets():
    return [
        LaunchAsset(
            ...
            what_achieved="Approve sessions from your phone.",
            why_built="No more sitting at the terminal.",
...
def test_launch_asset_messages_include_profile_and_caching():
    story = build_story("overseer", _events())
    system_blocks, messages = launch_asset_builder.build_messages(story, _profile())
    ...
    # The synthesis step must instruct grouping, not one-post-per-commit.
    assert "GROUP" in messages[0]["content"]


def test_parse_assets_extracts_json():
    text = (
        'noise [{"headline": "h", "what_achieved": "w", "why_built": "y", '
        '"source_refs": ["aaa111"]}] tail'
    )
    assets = launch_asset_builder.parse_assets(text, project="overseer")
    assert len(assets) == 1
    assert assets[0].headline == "h"
    assert assets[0].project == "overseer"


def test_post_messages_built_from_assets():
    system_blocks, messages = build_messages(_assets(), _profile(), count=3)
    assert any(b.get("cache_control") for b in system_blocks)
    content = messages[0]["content"]
    assert "Confident remote use" in content
    assert "USER VALUE" in content
    assert "3" in content
